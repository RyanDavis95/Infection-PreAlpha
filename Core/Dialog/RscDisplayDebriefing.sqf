...
switch _mode do {
	case "onLoad": {
		...
		if (_isDebriefing) then {
			...
			if(getTotalDLCUsageTime > 0) then //TODO-switch back on
			{
				...
				if(332350 in _nonOwnedUsedDLCs) then
				{
...

			{
				...
				_variableValue = _variableValue param [0,"",[""]];
	...
	*/
		...
		switch (ctrlIDC _tabButton) do
		{
			case IDC_RSCDISPLAYDEBRIEFING_DLCBUTTONALLASSETS:
			{
				...
				_assetsArray = ((configfile >> "CfgMods" >> "Kart" >> "Assets") call bis_fnc_returnChildren) +
					       ...
					       ((configfile >> "CfgMods" >> "Mark" >> "Assets") call bis_fnc_returnChildren);
			};
			...
			{
				//Mark
				_assetsArray = ((configfile >> "CfgMods" >> "Mark" >> "Assets") call bis_fnc_returnChildren);
			};
		};
			...
			if((count _reference) != 0) then
			{
				...
				if((_model != "") && ({_x == _model} count _nonOwnedUsedModels) > 0) then
				{
					...
					switch (_dlcType) do
					{
						case "kart":
						{
							_dlcLogo = [configfile >> "CfgMods" >> "Kart", "logoSmall", ""] call bis_fnc_returnConfigEntry;
						};

						case "heli":
						{
							_dlcLogo = [configfile >> "CfgMods" >> "Heli", "logoSmall", ""] call bis_fnc_returnConfigEntry;
						};

						case "mark":
						{
							_dlcLogo = [configfile >> "CfgMods" >> "Mark", "logoSmall", ""] call bis_fnc_returnConfigEntry;
						};
					...
					{ //Item creation
						...
						if(_idc == IDC_RSCDISPLAYDEBRIEFING_BUTTONPURCHASE) then
						{
							...
							switch (_dlcType) do
							{
								...
								case "mark":
								{
									_newControl ctrladdeventhandler ["buttonclick","openDlcPage 332350;"];
								};
	...
	case "onUnload": {
		...
		//--- Reset all UI layers, so the won't remain visible in MP menus
		titletext ["","plain"];
		cuttext ["","plain"];
		for "_i" from 0 to 100 do {_i cuttext ["","plain"];};
	};
};
